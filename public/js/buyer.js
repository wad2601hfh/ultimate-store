...
function renderAppend(messages) {
    messages.forEach(msg => {
        ...
        if (msg.type === 'text' || msg.type === 'system') {
            ...
            const storeTag = msg.store ? `<span class="store-badge">🏪 ${msg.store}</span>` : '';
            row.innerHTML = `
                <div class="card">
                    ${storeTag}
                    <h3>${msg.foodName}</h3>
...
// 2. SENDING (FIXED: No double messages)
async function sendText() {
    ...
    // Send to backend
    await fetch('../backend/api/chat.php', { 
        method: 'POST', 
        body: JSON.stringify({ action: 'send_text', sender: 'buyer', text }) 
    });

    // Force immediate update from server
    poll();
}
...
function renderCart() {
    ...
    cart.forEach(item => {
        total += item.price;
        list.innerHTML += `
            <div style="display:flex; justify-content:space-between; padding:10px 0; border-bottom:1px dashed #333">
                <span>${item.name}</span>
...
async function pay() {
    ...
    setTimeout(() => {
        ...
        toggleModal(false);
        btn.innerHTML = "INITIATE TRANSFER";
